""" WollPay Clients Base (Version 00.04) """
...
def force_get_carrent_date(date):
    """Get current date (Poland) if press "ENTER".
    date - parameter(Date to check).
    """
    if len(date) < 1:
...
def date_validation(date):
    ...
    if len(date) == 10:
        ...
        date = force_get_carrent_date(date)
...
def number_validation(number, max_number):
    ...
    number = remove_whitespaces(number) # Removing whitespaces.
...
while i != 'q':
    ...
    print('Hello Host! \nYou run version 00.04 of program with partly validation.')
    ...
    dependent = input('Dependent: ') # Dependent input and validation.
    if len(dependent) <=1:
        pass # For blank mode.
    else:
