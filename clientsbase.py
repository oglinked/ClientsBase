""" WollPay Clients Base (Version 00.06 of the clientsbase.py). """
...
def remove_cyrillic_and_tabs(string, message=message_latin):
    """Code to test Cyrillic letters presence & remove tabs."""
...
def valid_isalnum(str_to_valid):
    ...
    Returns Latin letters on the upper case.
    """
    if str_to_valid.isalnum():
        pass
    else:
        print('Error: Only Latin letters and decimal digits \
...
def str_valid_to_upper(str_to_valid):
    ...
    str_to_valid - parameter (Input string to validate).
    """
...
def valid_decimal(number):
    ...
    number - parameter (Inputted string to validate).
    """
    number = cyrillic_presence_test(number, message_cyrillic)
    number = remove_tabs_and_whitespaces(number)
    if number.isdecimal():
...
def check_letters(str_to_valid):
    ...
    str_to_valid = cyrillic_presence_test(str_to_valid)
    str_to_valid = remove_tabs(str_to_valid)
    result = True # Initializing: True -> no letters in string.
    for i in str_to_valid:
        if i.isalpha(): # If string has letters -> False.
            result = False
    return result
...
def date_validation(date):
    """ Date validation.
    date - parameter (Inputted date to validate).
    """
    date = cyrillic_presence_test(date, message_cyrillic)
    date = remove_tabs_and_whitespaces(date) # Remove "\t" and " ".
    # Testing the length of the field.
    if len(date) == 10: # len('mm/dd/yyyy') == 10
        pass
    else:
        ...
        if date == '': return date
    # Testing the absence of Latin letters.
...
base_path = './clients_base.csv' # The relative path to the clients_base file.
...
while i != 'q':
    ...
    print('Hello Host! \nYou run version 00.06 of the clientsbase.py program.')
